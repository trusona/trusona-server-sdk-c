...
START_TEST(will_return_correct_value_for_regular_files)
{
...
START_TEST(will_return_correct_value_for_sticky_files)
{
...
START_TEST(will_return_predefined_value_for_nonexistant_files)
{
  ck_assert_int_eq(file_perms("no-such-file.txt"), -1);
}
END_TEST;

START_TEST(will_return_predefined_value_for_symbolic_links)
{
...
Suite *utils_suite(void)
{
  ...
  suite = suite_create("File Permissions Tests");
  tests = tcase_create("tests");

  tcase_add_checked_fixture(tests, setup, teardown);
  tcase_add_test(tests, will_return_predefined_value_for_nonexistant_files);
  tcase_add_test(tests, will_return_predefined_value_for_symbolic_links);
  tcase_add_test(tests, will_return_correct_value_for_regular_files);
  tcase_add_test(tests, will_return_correct_value_for_sticky_files);
